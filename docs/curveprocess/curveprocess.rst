.. _curveprocess:

.. include:: ../symbols.rst

The ``curveprocess`` program
============================

.. autosummary::
   :toctree: generated


The ``curveprocess`` program extracts and calculates DUT parameters from the curve data in ``pypsucurvetrace`` files:

   * Gate/grid voltage (|Vg|) or base current (|Ib|) needed for a given operating point (|U1| and |I1|)
   * Transconductance (|gm|) or current gain (|hfe|) at a given operating point (|U1| and |I1|)
   * Output conductance (|gm|)

The ``curveprocess`` documentation can be accessed from the ``curveprocess`` program directly:

.. code-block:: console

   curveprocess --help

Examples
--------
UNDER CONSTRUCTION...

