...
The procedure implemented in the |curvematch| program to calculate and report the similarity between the curve sets is as follows:
   ...
   #. The RMS value of the |Dij| values is reported. A low RMS value indicates that the two curve data sets are similar to each other (i.e., the surfaces in the 3-dimensional plot are in close proximity to each other).
